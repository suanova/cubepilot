...
func TestDefaultReadAllowlistShape(t *testing.T) {
	...
	// delete ...`) must never be allowlisted, and neither may bins with
	// mutating flags such as `date -s` (issue #20 code review).
	for _, e := range al {
		for _, banned := range []string{"env", "xargs", "sh", "bash", "nohup", "timeout", "date"} {
			if e.Pattern == banned {
