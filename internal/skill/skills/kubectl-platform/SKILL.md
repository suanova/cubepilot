...
3. Validate against the server before creating (needs no extra permission; use the user identity): apply with `--dry-run=server` and iterate on the error — the API server lists the required fields (e.g. `spec.image`, `spec.resources`) and rejects unknown ones:
