...
// explicitly set selectedModel, and then it is the <provider>/<modelId> ref
// some provider of the agent definition serves. Empty means "no override --
// use the runtime's configured primary model", so the deployer's provider
// config decides the default and no provider-key naming convention is
// required. An explicitly selected ref the definition does not serve is an
// error (fail-closed, never a silent fallback); an empty selection is not an
// error.
