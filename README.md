...
`AgentTemplate` inline `models`. No platform default LLM is assumed: the chart
ships a model-less `agent-for-cloud` template, and models are added from the
Portal (Agent Config -> LLM Config). To seed a ready default model at deploy
time, configure an endpoint + model together with the `cubepilot-llm` apiKey
Secret (`scripts/setup.sh` does this for DeepSeek). To use a specific model
once several exist, select it in the Portal (Agent Config) or set
...
automatically. There is no forced default model: `agent-for-cloud` is created
model-less and every LLM (the "default" included) is declared the same way -- as
a model on the template plus its apiKey Secret.

To seed a DeepSeek default at deploy time, `scripts/setup.sh` configures an
endpoint + model and creates the `cubepilot-llm` Secret:
...
leaves them empty for a model-less install. To add or change models after
install, use the Portal (Agent Config -> LLM Config): give a model name, an
OpenAI-compatible endpoint, and an apiKey (leave empty for a public model).
The operator re-renders the gateway and the model becomes usable and
