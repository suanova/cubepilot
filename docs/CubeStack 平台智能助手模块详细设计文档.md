...
**包含**：对话式智能助手、平台知识问答（RAG）、自然语言操作执行（受权限与审批收敛）、智能运维（AI Ops：集群健康巡检、推理服务自动验证、自动化运维 Workflow、故障自动诊断与 RCA、自动修复）、Agent 运行时与工具（Skill）体系、工具级权限与敏感操作审批、操作审计、通知与消息。
...
|Human-in-the-loop|人在回路，敏感/高风险操作必须人工审批后执行|
|审批任务|一次敏感工具调用产生的待审批事项，由有权限用户在 Portal 或 IM 中审批|
...
|通知与消息|巡检报告、验证结果、审批请求、RCA 报告推送|所有用户|
...
|角色|助手可用工具|敏感操作|说明|
|---|---|---|---|
|平台管理员|全部（含高风险）|审批通过后执行|可管理工具策略、Workflow、模型配置|
|租户管理员|租户内全部（含写操作）|需会话确认/审批|可管理租户内资源操作|
|开发人员|项目内只读 + 操作类|写操作需会话确认|查询与操作本人有权限的项目资源|
...
|平台 RBAC（租户/项目/角色）|工具调用前的权限判定依据|
|Kubernetes API|集群/节点/Pod/事件查询|
...
|会话存储|平台数据服务（PostgreSQL）+ Redis|会话、消息、审批、审计持久化；热点上下文缓存|
|实例数据目录|共享存储（Lustre / NFS / PVC）|每用户一个目录，持久化实例 HERMES_HOME 与记忆文件；实例回收后目录保留，重建时重新挂载|

> **选型说明**：选择 OpenClaw 而非自研，是为了复用成熟的 Agent 编排与多通道接入能力，把研发投入集中在平台特有的「工具授权、审批、审计、运维资产」上；这些资产与运行时解耦（工具层为标准 MCP，流程为声明式 CRD），Agent 以每用户实例形态部署，隔离边界为物理实例，故 OpenClaw / Hermes / 自研均可经 Agent Runtime Adapter 平滑替换（见 4\.2\.5）。
...
- **模型路由 / 知识库 / 通知**：LLM 请求路由（限流与降级）、对话前 RAG 检索注入、报告与审批推送
...
|安全约束|禁止事项清单（越权、敏感信息外发等）|护栏，配合工具授权兜底|
...
|`job.submit` / `job.cancel` / `inference.deploy` / `inference.scale`|创建/取消任务、部署/扩缩推理服务|L1|写操作，需会话确认|
|`inspection.run` / `workflow.run` / `notification.send`|触发巡检、执行 Workflow、发送通知|L1|按声明判定审批级别|
|`pod.restart`|重建异常 Pod|L2|高风险，需审批|
...
|授权级别|定义|执行方式|
|---|---|---|
|L0 · 只读|不改变平台状态的查询类工具|Agent 直接调用，不额外拦截|
...
flowchart LR
    ...
    D -->|批准| E[执行工具调用]
    D -->|拒绝| F[终止并反馈用户]
    D -->|超时未处理 15min| G[任务超时<br/>默认拒绝并通知]
    E --> H[结果回填 Agent]
```
...
|错误类型|说明|Agent 处理建议|
|---|---|---|
|`PERMISSION_DENIED`|无权限|说明权限范围，引导授权或联系管理员|
|`APPROVAL_REQUIRED`|需要审批|引导用户发起审批并告知状态|
...
**触发**：推理服务上线时自动执行、每日定时抽检、对话手动触发。**输出**：`ServiceValidation` 结果（通过/失败/警告 + 明细），供 Portal 展示，并作为「一键上线」审批依据。

### 4\.4\.3 自动化运维 Workflow

**能力**：将常见运维流程封装为声明式 Workflow，支持预置与自定义扩展，自动执行。

**Workflow 声明（`AssistantWorkflow` CRD）**：`spec.trigger`（manual/cron/event）、`spec.cron`、`spec.event`、`spec.steps`（有序步骤：引用工具 + 参数模板）、`spec.approvalPolicy`（L0/L1/L2 或按步骤指定）、`spec.timeout`（默认 30min）、`spec.retryPolicy`、`spec.notify`、`spec.scope`（all/tenant/project/node-pool）。
...
|`pod-auto-restore`|异常 Pod 自动重建（限 CrashLoopBackOff / ImagePullBackOff 且非用户任务核心）|L2|
|`training-job-cleanup`|过期/已完成训练任务与中间产物清理|L2|
...
|Pod CrashLoopBackOff（判定为镜像/配置问题，非用户代码问题）|生成修复方案，执行需审批|L2|
|GPU 降级/故障|隔离故障 GPU，通知，不自动重启节点|L1（会话确认）|

**安全护栏**：默认关闭，由管理员在 ToolPolicy 中显式开启；仅对白名单 Workflow 生效（`pod-auto-restore` 等）；每次修复动作进审计并推送结果；修复失败不静默——升级为告警并转人工。
...
|「重启那个一直崩的 Pod」|pod.restart|L2|

**操作收敛原则**：只执行用户**明确要求**且**有权限**的操作，不做推测性写操作；高风险操作永远走审批，禁止静默执行；操作前展示「计划执行的工具与参数」供用户确认（L1）或审批人审查（L2）；工具调用过程与结果透明可解释。
...
|渠道|用途|实现|
|---|---|---|
|平台消息中心|巡检报告、验证结果、审批请求、RCA 报告（站内）|调用平台消息中心 API|
|邮件|重要报告（P0/P1）与审批请求|SMTP 适配|
|飞书 / 企业微信（可选）|ChatOps 入口与审批处理|IM 机器人适配层|

消息按类型预置模板（巡检摘要、验证结果、审批待办、RCA 结论），支持管理员自定义；内容遵循「先结论后证据、可追溯」原则，重要通知附操作者、时间、资源 ID。
...
||/api/v1/assistant/tool-calls/\{id\}/confirm|POST|L1 写操作会话确认|
|审批（第二阶段）|/api/v1/assistant/approvals|GET|待我审批 / 我发起的列表|
||/api/v1/assistant/approvals/\{id\}|GET|详情（含操作与资源快照）|
||/api/v1/assistant/approvals/\{id\}/approve|POST|批准|
||/api/v1/assistant/approvals/\{id\}/reject|POST|拒绝（需填写原因）|
...
|管理|/api/v1/assistant/admin/tool-policies|GET / PUT|工具授权表管理（平台管理员）|
...
> 前端通过 SSE 消费事件流；`tool_call` 事件用于渲染「Agent 正在做什么」的透明过程；L1 写操作服务端先行拦截并返回 `event: confirm_required`（用户确认后继续）；L2（第二阶段）返回 `event: approval_required`。
...
**存储策略**：会话、消息、审批、审计等高写入量数据存**平台数据服务（DB）+ Redis**；巡检报告、验证结果、RCA 报告、Workflow 等「运维资产」以 **CRD**（`assistant.suanova.io/v1alpha1`）承载，可版本化、可审计、可恢复。
...
**ToolCallRecord**（审计性质）：`id`、`conversation_id / message_id`、`tool`、`args`、`level`（L0/L1/L2）、`status`（pending/executed/rejected/failed/timeout）、`result`(json)、`duration_ms`、`created_at`。

**ApprovalTask**：`id`、`tool_call_id`、`tool / args_preview`、`resource_snapshot`(json)、`applicant_id`、`approver_id`、`status`（pending/approved/rejected/timeout）、`comment`（拒绝必填）、`expire_at`（默认 15min）。审批任务随 L2 审批闭环在第二阶段启用（见 14.2）；第一阶段仅记录 ToolCallRecord 审计。
...
> 阶段说明：7.1 为第一阶段（对话 + L0/L1 工具）；7.2 审批流、7.3 巡检、7.4 验证、7.5 RCA 为第二阶段能力。
...
```mermaid
stateDiagram-v2
    [*] --> pending: Agent 发起调用(L1/L2)
    pending --> executed: 会话确认(L1)/审批通过(L2)
    pending --> rejected: 审批拒绝
    pending --> timeout: 审批超时
...
stateDiagram-v2
    ...
    running --> awaiting_approval: 步骤触发 L2 审批
...
- 所有 L2 操作必须审批，审批任务含资源快照与影响面说明
- 审批链默认单级（同租户上级/平台管理员），超时默认拒绝（15min），动作全程留痕；多级顺序审批为第三阶段扩展
...
- 全链路审计：会话消息、工具调用（含参数）、审批动作、Workflow 执行、配置变更、知识库变更
...
|知识库向量存储 / Redis|文档数 × 分块 × 维度；热点上下文 + 审批缓存|按规模规划，支持水平扩展|
...
**功能**：对话问答正确返回；只读/写/高风险工具按授权级别正确执行与拦截；审批（批准、拒绝、超时默认拒绝、重复审批幂等）；会话创建/续接/归档/并发限制；**实例生命周期**（冷启动、预热池命中、闲置回收、重建后会话/记忆恢复、手动回收）；**物理隔离**（用户 A 实例无法访问用户 B 数据目录/记忆/会话；实例凭据越权操作被授权门拦截）；巡检/验证/RCA/自动修复全流程；Workflow 注册执行与失败重试。
...
> 原则：第一阶段只做 MVP——非必要能力一律后置；审批闭环、AI Ops、自动修复、IM 通道均不在第一阶段实现。
...
- 经 Agent Runtime Adapter 平滑切换运行时（OpenClaw → Hermes → 自研），隔离与记忆机制不变
- 多级审批链、审批人矩阵；事件驱动自动化闭环（告警 → 诊断 → 处置 → 复盘）
- 多 Agent 协同（巡检 / 诊断 / 执行 / 审批分工）
...
|ISSUE-005|审批人路由规则（按资源/租户/角色匹配）细化|中|待设计|含多级审批链定义|
