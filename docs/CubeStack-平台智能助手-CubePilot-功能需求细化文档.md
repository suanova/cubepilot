...
**定位：** 需求层文档，细化助手模块功能需求并按交付批次标注阶段，含各业务模块 AI Agent 能力与对接设计（[§8](#8-各模块-ai-agent-能力与对接设计)）；设计细节见《CubePilot 模块设计文档 v0.1》与《平台智能助手详细设计 v0.4》，本文档不替代设计文档。
...
**版本：** v0.1（合并自独立文档）　**定位：** 梳理平台各业务模块的 AI Agent 能力，说明各模块如何与 CubePilot 基座对接。基座能力见 [§3](#3-功能需求按功能域)/[§4](#4-非功能需求)，本文档本章不重复定义基座。
...
1. **能力 API 化**：模块能力遵循 API/CLI-First 原则，提供标准化 RESTful / OpenAPI / CLI 接口，Agent 可编程调用。
...
- **C. 数据开放 + 基座推理**：模块只暴露数据/API，由基座 LLM 直接分析——属于「能力目录工具化」；关键诊断场景因纯 LLM 分析可靠性不足，倾向用 A/B。

> **待定（Q-010）**：模块「自带 Agent」的最终形态——独立 Agent 实例，还是复用基座 Agent + 模块注册领域技能（倾向后者，避免每模块一套实例）。
