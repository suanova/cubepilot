...
>
> **与既有文档的关系**：
> - 《CubePilot-Module-Design.md》（v0.2）的 M1~M6 功能域、E1~E5 扩展点、CRD 与安全/部署设计在本文档框架下**重组归属，内容不推翻、仅重新分层**；本文档是 v0.2 的框架级演进候选稿。
> - 《CubePilot-Functional-Requirements.md》（v0.5）仍是**需求唯一真源**，本文档与 `FR-M{域}-{序号}` / `NFR-{序号}` 对齐；涉及需求调整时同步更新需求文档。
