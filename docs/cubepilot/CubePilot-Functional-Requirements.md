...
## 1.1 前置决策（2026-08-15 已与 PM 确认）

以下决策位于需求上游，直接决定需求与方案形态；与既有需求冲突之处需按下表及本表后「与既有需求的冲突」同步调整。

| # | 问题 | 为什么决定需求走向 | 结论 |
|---|---|---|---|
| 1 | LLM 规格：私有化部署为既定前提（无需讨论）；推荐的最低模型要求是否为 DeepSeek V4 Flash 同级？该规格是否认可、算力是否到位？ | 模型规格决定编排与工具调用能力上限 → 可承诺场景与验收标准，是全部需求的前提 | 最低规格确认为 DeepSeek V4 Flash 同级（避免弱模型影响体验）；一期不强制自建——可自建（默认 W8A8/FP8 部署，64G 级 GPU 推荐 6 张、生产富余 8 张）或接入公司已有/公网模型 |
| 2 | IM：接不接飞书/企微？双向对话还是只发通知？ | 双向对话需常驻网关接收消息（决定有无常驻组件、Agent Pod 是否常驻）；只发通知仅是出站调用，无常驻负担 | 一期以页面交互为主、预留飞书/企微接入能力；完整 IM 接入二期 |
| 3 | 用户量级：总用户量级与同时使用助手峰值？（50 以内 / 几百 / 上千） | 决定 Agent 实例按需拉起即可，还是需要池化与容量规划 | 几百以内（不会上千）；一期实例常驻（注册即拉起、持续运行），二期再考虑按需启停 |
| 4 | 隔离粒度：每用户独立实例（Pod 级）是否硬性要求？共享实例 + 逻辑隔离可否接受？ | 隔离粒度 × 用户量级 = 资源总成本，与 #3 一起确认 | 每用户一个独立 Agent（Pod 级隔离），各自绑定自身 kubeconfig 权限 |
| 5 | HITL：写操作（create/update/delete）是否需人工确认？全部确认 / 仅删除类确认 / 直放不确认？ | 决定交互形态与 Agent 工具接入形态（确认钩子的实现位置） | 一期不实现完整 HITL：写操作直放，可借运行时原生「命令执行审批」能力简单处理（审批提示经 CLI 或 Portal 桥接）；二期完善确认机制并记录 Agent 执行过程 |
| 6 | 运行时：OpenClaw / Hermes / 自研？有无否决性约束？ | 决定技术栈与运行时替换接口的投入 | 优先 OpenClaw（约 80% 倾向、未最终定论，Hermes 备选）；抽象一层 Agent Runtime 适配层以便更换，一期不绑定死 |
| 7 | 界面：是否对用户开放运行时原生页面？ | 开放 → 原生页与 Portal 双入口并存；不开放 → 入口收敛于 Portal，对话页需自建 | 不开放运行时原生页面；统一 UI 自行开发/改造（Portal 入口） |

**其他会议结论（未列入上表）**：
- 一期不开放用户注册，写死一个管理员用户；多用户 + 每用户独立 Agent 实例后续支持。
- 一期工具以 kubectl 为主（Agent 调 K8s）；Prometheus / GPUStack 等经 MCP 或 skill 方式后续接入；各模块自行梳理常见场景、提供 skill/文档指导 Agent 操作（一期形成模板）。
- 审计与 Agent 执行过程记录放二期，一期简化、不引入复杂存储。
- 对话历史仅保留近期（48~72 小时），不长期存储。
- Agent 自身数据（memory/配置）经挂载 PV 持久化；定时任务功能一期保留，其数据持久化一期简化（挂 PV/平台存储），专门的数据建模二期设计。
- 开发方式：敏捷，先做简单可用版本，再按实际需求迭代。

> 与既有需求的冲突已按下表结论同步调整（见 §3~§5 对应条目）。
...
flowchart TB
    ...
    M4["M4 定时 AI 任务域<br/>任务模板与任务 · 分级报告"]
...
> 说明：Portal 除对话页（M1）外，还提供任务报告（M4）、审计查询（M5）、Agent 配置（M2）等非对话入口；**定时 AI 任务（M4）由调度器定时触发，经 M2 Agent 实例执行**——以创建者身份跑任务（巡检是预置任务之一），经 M3 工具域访问平台能力层，结果写报告供 Portal / API 展示；阶段三起报告供 Agent 消费，用于 RCA 根因分析 / 自动修复 / 预测性运维（FR-M4-012~014）。
...
| FR-M1-005 | 上下文组装：平台侧要素（操作者身份、能力发现 FR-M3-006、确认规则〔阶段二起〕）为实例级配置、启动时加载；每次请求仅动态组装新消息与会话引用——执行上下文（历史窗口 / 超窗压缩摘要 FR-M1-010）由 Agent 运行时维护，平台不重传历史 | 上下文含平台侧要素；新消息经会话引用路由至对应会话并正确续接 | 一 |
...
> 编排循环（理解→规划→工具调用→评估→汇总）及跨模块任务编排由 Agent 运行时原生提供，配合能力发现（FR-M3-006）即可实现，非自研项。

| 编号 | 需求 | 验收要点 | 阶段 |
|---|---|---|---|
| FR-M2-001 | 操作者间物理隔离（多用户体系就绪后按每用户独立实例/数据目录隔离，机制承接 NFR-002） | 多用户就绪后操作者 A 无法访问 B 的数据/会话/记忆 | 二 |
| FR-M2-002 | Agent 实例生命周期：一期常驻运行（拉起后持续运行）、异常自动恢复；二期按需启停、闲置回收资源 | 一期常驻自愈；二期按需启停、空闲回收 | 一 |
| FR-M2-003 | LLM 路由：模型无关，默认接入平台私有化模型（一期可不自建、接入公司已有/公网推理服务），支持配置平台外 LLM | 换模型不破坏对话功能 | 一 |
| FR-M2-004 | 实例可重建且不丢失会话与记忆 | 重建后会话与记忆完整 | 一 |
| FR-M2-005 | **用户 Agent 配置管理**：模型选择/工具开关/系统提示词，持久化并即时生效；能力发现（FR-M3-006）作为工具开关与系统提示词的一部分，实例启动时加载 | 改配置即生效、重启保留 | 一 |
...
| FR-M2-009 | Agent Runtime 适配层：抽象适配接口、一期落地，运行时可替换、不锁定单一运行时实现 | 更换运行时不需重写业务逻辑 | 一 |
| FR-M2-010 | 多模型路由与切换（按会话/用户，与 FR-M2-005 联动） | 切换后对话用新模型 | 三 |
| FR-M2-011 | 长期记忆体系（跨会话记忆/偏好沉淀） | 新会话引用历史偏好 | 三 |

## 3.3 M3 工具域（职责：平台能力操作化、能力发现、确认判定）

| 编号 | 需求 | 验收要点 | 阶段 |
|---|---|---|---|
| FR-M3-001 | 平台资源操作：Agent 以用户身份操作平台资源（含全部 CRD），权限由平台 RBAC 强制；读操作直放，写操作**阶段一直放执行**（可借运行时原生「命令执行审批」简单处理，kubectl 写命令命中时暂停审批）、**阶段二起命中确认规则后需确认**（FR-M3-002/003） | 读写均以用户身份执行；无权限被 RBAC 拒绝 | 一 |
...
| FR-M3-006 | 平台能力发现：generic 工具（kubectl 执行 + schema 发现）零登记覆盖全部 CRD；模块需要语义时写**可选** Atomic 薄覆盖（用途/说明/示例），需要领域流程时写 Domain 能力 | generic 工具可直接操作任意 CRD；Atomic/Domain 内容随实例配置注入、Agent 据此正确调用；新增 CRD 无需登记 | 一 |
| FR-M3-007 | 预测性运维工具（容量预测/利用率趋势/告警关联）——**工具形态**：供对话与任务按需调用 | 返回分析结果 | 三 |

## 3.4 M4 定时 AI 任务域（职责：任务模板 · 定时任务 · 分级报告）

| 编号 | 需求 | 验收要点 | 阶段 |
|---|---|---|---|
| FR-M4-001 | 任务调度机制（FR-M4-002 用户任务与预置巡检的运行底座）：支持每日定时 + 手动/API 触发（实现方式不限），到点拉起执行；以创建者身份执行、权限与创建者一致 | 到点/手动可触发；无权限项执行时被拒绝并标注 | 一 |
| FR-M4-002 | 定时/触发任务（用户级）：用户创建定时/手动/事件触发的任务——可引用**任务模板**（平台/管理员维护的预置任务定义，巡检为内置模板之一）或直接自定义指令，以创建者身份执行、结果按用户隔离；阶段一读写均直放、不二次确认，创建时明确提示「将以你的身份执行、不再确认」 | 任务到点/触发即执行；创建时可见直放提示；无权限项被拒绝并标注；任务/结果仅创建者可见 | 一 |
...
| FR-M4-007 | AI 智能巡检：自主探索集群状态，发现预置巡检项之外的异常（配置漂移、资源浪费、跨资源关联异常等），输出结构化发现 + 自然语言描述 | 对注入的故障样本/已知异常场景，能发现预置项未覆盖的异常；发现附带证据链与自然语言说明；置信度不足的发现标注「AI 疑似，需人工复核」 | 一 |
| FR-M4-008 | 推理服务自动验证：启动/健康/推理请求/结果校验/响应时间/GPU 利用率 6 项 | 输出验证报告 | 二 |
| FR-M4-009 | 任务模板：可自定义任务模板——声明任务指令内容（可参数化）、所需权限提示、输出形式；巡检为预置模板之一；**模板不含创建者——模板只是「做什么」的定义，不包含执行主体** | 可定义模板，模板可更新，可基于模板创建任务 | 二 |
| FR-M4-010 | 告警触发自动诊断（监控告警联动拉起诊断任务） | 告警联动拉起诊断任务 | 二 |
| FR-M4-011 | 通知推送（报告/验证结果→站内+邮件+飞书）——按订阅规则多通道自动推送；区别于阶段二的告警触发式推送（FR-M2-008 + FR-M6-005） | 多通道可达、订阅生效 | 三 |
| FR-M4-012 | RCA 根因分析（监控/日志/事件→结构化报告） | 输出含证据链 RCA | 三 |
| FR-M4-013 | 自动修复 Auto-Fix（可安全自愈异常受控修复，需确认） | 安全场景自愈、危险不自动 | 三 |
| FR-M4-014 | 预测性运维（容量预测/异常趋势预警）——**执行形态**：定期产出预测/预警，调用 FR-M3-007 工具能力 | 输出预测与预警 | 三 |

> 授权约定：定时 AI 任务（FR-M4-001~007）均为用户级能力——以创建者身份执行、权限与创建者一致；**任务模板仅定义「做什么」，不含创建者**——某次定时/触发执行是用户的任务，巡检是预置任务模板之一，全集群巡检需集群级只读权限（通常运维/管理员具备），普通用户可建任务但范围限于自身 project，无权限项执行时被拒绝并标注。模板的「所需权限」为提示性说明，非授权机制。

## 3.5 M5 审计域（职责：工具调用与确认记录）

| 编号 | 需求 | 验收要点 | 阶段 |
|---|---|---|---|
| FR-M5-001 | Agent 行为可溯源：每次工具调用（工具/参数/结果/时序）在 Agent 运行轨迹中完整可查；HITL 确认决定留痕（操作人/规则/决定/时间）；需长期留档的可导出归档 | 任意一次执行可回放工具调用序列；确认记录可查 | 二 |
| FR-M5-002 | 审计查询 API：按会话/用户/时间定位工具调用轨迹与确认记录（/audit-logs） | 可筛选查询 | 二 |
| FR-M5-003 | 审计不阻塞对话主链路 | 审计故障不影响对话 | 二 |
| FR-M5-004 | 审计治理界面（轨迹查询/回放、确认记录、导出） | 界面可用 | 二 |
...
| FR-M6-003 | 助手 LLM 推理服务：一期不强制自建——可自建（默认 W8A8/FP8 部署，64G 级 GPU 推荐 6 张、生产富余 8 张）或接入公司已有/公网推理服务；独立部署、可弹性扩缩、内网运行 | 可用、可扩缩、内网部署 | 一 |
| FR-M6-004 | 每用户数据目录：Agent 实例工作目录（memory/配置经 PV 持久化），多用户体系就绪后按用户隔离（用户间不可互访） | 实例可读写自身目录 | 一 |
...
| NFR-016 | 助手 LLM GPU：独立推理节点，默认 W8A8/FP8 部署（DeepSeek V4 Flash 同级），64G 级 GPU 推荐 6 张、生产富余 8 张 | 一 |
| NFR-017 | 单 Agent 实例资源规格基线：0.5~1 vCPU / 1~2GB 每实例（实例池总量上限与回收见 NFR-015） | 一 |
...
|---|---|---|---|
| **一 · 首批必须** | 最小可用对话助手 | 对话闭环 + Portal 对话页（FR-M1-001、003~005、007）；Agent 实例与配置管理 + Runtime 适配层（FR-M2-002~005、009）；平台资源操作 + 能力发现（FR-M3-001、006）；**定时 AI 任务（含巡检模板）**（FR-M4-001~005、007）；平台集成基础（FR-M6-001~004）；可观测性接口（NFR-011~013）；安全/性能基础（NFR-002~004、006~007、010、016~017） | 自然语言创建开发环境/查询/操作推理服务（写操作直放、可借命令执行审批）；Agent 依据能力发现正确选择 CRD 并操作；实例常驻自愈；预置+AI 巡检每日出报告；/metrics+日志关联字段；首 Token<3s |
| **二 · 次批** | 安全与 AI Ops 扩展 | 会话限制/多入口/移动端（FR-M1-006、011~012）；主动告警与报告（FR-M2-008）；实例指标/告警（FR-M2-006、NFR-014）；RAG 问答与上下文注入（FR-M1-008、M2-007）；确认规则/HITL/dry-run/错误解析（FR-M3-002~005）；任务增强（FR-M4-006、008~010）；按需启停/闲置回收（FR-M2-002）；审计（FR-M5-001~004）；通知通道（FR-M6-005）；富文本/截断（FR-M1-009~010）；身份与多用户隔离（NFR-001、FR-M1-002、FR-M2-001）；性能达标（NFR-008~009） | 写操作命中 HITL；实例按需启停；审计可查；RAG 答平台问题并溯源；自动验证输出报告；预置+自定义任务模板可编排 |
| **三 · 后续** | 智能演进 | 多模态/长期记忆（FR-M1-013~014、M2-011）；多模型路由（FR-M2-010）；RCA/自动修复/预测运维（FR-M4-012~014、M3-007）；通知推送（FR-M4-011）；审计合规报告（FR-M5-005）；限流（NFR-005）；实例池容量（NFR-015） | RCA 自动生成含证据链；多模型切换可用；长期记忆跨会话引用 |

## 5.2 阶段一关键决策

- **阶段一为单管理员用户**：一期不开放用户注册、写死一个管理员用户；平台多用户/租户体系尚未就绪，身份与授权（NFR-001）随平台用户体系在阶段二落地；物理隔离（NFR-002）阶段一通过每用户独立实例实现，多用户上线后自然延续。
- **阶段一实例常驻**：实例拉起后常驻运行、异常自愈，不做闲置回收；按需启停、闲置回收（FR-M2-002）放二期（对齐「用户量几百以内、一期常驻」结论）。
- **阶段一写操作不设完整 HITL**：直放执行，等效用户本人操作；可借运行时原生「命令执行审批」简单处理（kubectl 写命令命中时暂停审批，审批提示经 CLI 或 Portal 桥接），不建设完整的确认规则与确认界面（FR-M3-002~004 阶段二）；由「凭据直连 + API Server RBAC + K8s Audit Log」兜底，建议阶段一凭据最小权限。风险：LLM 幻觉可能触发未预料的写操作。
- **阶段一不建业务工具与工具级权限系统**：平台资源均为 CRD，Agent 可通过平台 API 覆盖主要操作；阶段一通过平台能力发现（FR-M3-006）注入 Agent 上下文，Agent 依据 schema 直接操作 CRD；非 K8s 数据源（GPU 指标等）阶段二起按需接入。
...
- **工具调用延迟 SLO 刻意后置**（NFR-008 阶段二）：阶段一工具链以 kubectl 为主、单次调用时延容忍度高，暂不立 SLO；阶段二随确认/审计体系完善一并落地。
- **可观测性接口阶段一预留、能力后置**（NFR-011~013）：埋点/日志关联字段/trace 约定是写码规范，事后补成本高；Loki/Perses/告警/追踪后端阶段二后置。

## 5.3 扩展点

知识注入 → RAG（FR-M1-008）· Agent Runtime 适配层（FR-M2-009，一期落地 → 阶段三运行时替换）· 定时任务骨架（→ 推理服务验证/任务模板）· 平台操作底座 + 能力发现（→ 后续按需扩展非 K8s 工具）· **任务报告 → Agent 消费（阶段三 RCA/自动修复/预测运维，FR-M4-012~014）**。
...
| Q-010 | Atomic/Domain 能力的内容约定与模块接入方式（谁写、是否审核） | §8.3、FR-M3-006 | 中 |
...
| 平台能力发现（generic 零登记 + 可选 Atomic/Domain） | FR-M3-006 | 一 |
| 跨模块任务编排（一句话串联多模块操作） | Agent 原生能力（配合能力发现即实现） | 一 |
...
2. **能力发现**：模块 CRD 经 generic 工具（FR-M3-006）零登记自动可用；需要语义时写 Atomic 薄覆盖（用途/说明/示例），需要领域流程时写 Domain 能力。
3. **工具化调用**：Agent 依据 schema/能力经平台资源操作（FR-M3-001）调用；阶段二起复杂操作先预览影响（FR-M3-004），高风险操作需 HITL 确认（FR-M3-003）。
...
| **CRD 直操作** | 模块的**操作类**能力（创建环境、提交训练、部署推理等） | 模块提供 CRD，Agent 经 generic 工具直接操作（需要语义写 Atomic） |
| **模块自带** | 模块的**领域智能**（数据画像、模型推荐、评估解读、成本优化、告警关联等） | 由模块实现，基座提供对话/推理外壳 |

**§8.4 归属取值说明**（固定词汇，表内用「+」表示多方协作）：

- **CRD 直操作**：模块提供 CRD，Agent 经 generic 工具直接操作（需要语义写 Atomic；纯问答类可标注「基座 LLM 问答」）；
...
> - **A. 能力 API 化**：模块封装领域逻辑为 API → 经 generic 工具直接调用
...
|---|---|---|---|
| 自然语言创建环境 | DevEnvironment CRD / 环境 API | CRD 直操作 | 一 |
| 环境诊断（CUDA 冲突/OOM/依赖冲突提示+修复建议） | 环境状态、GPU/显存指标、日志 | 模块自带 | 一 |
| 环境推荐（按历史使用+当前任务） | 用户历史、任务上下文 | 模块自带 | 三 |
| 一键复刻（打包镜像+推私有仓库+分享配置） | 镜像构建/推送 API（Harbor） | CRD 直操作 | 二 |
...
| 数据问答（条数、label 分布差异等查询） | 数据集元数据/统计 API | CRD 直操作（基座 LLM 问答） | 二 |
...
| 自动模型测试（Playground 跑用例生成体验报告，报告由模块自身生成、不依赖基座报告） | 推理服务、测试用例集 | 模块自带 | 一 |
| 模型血缘问答（lineage 查询） | Model/Lineage 元数据 | CRD 直操作（基座 LLM 问答） | 二 |
...
| 能力 | 依赖的平台 API/数据 | 实现归属 | 阶段 |
|---|---|---|---|
| 自然语言提交训练 | TrainingJob CRD / 训练 API | CRD 直操作 | 二 |
...
| 自动排障（服务异常→日志→定位→方案→执行：阶段一直放执行、阶段二起写操作按 FR-M3-003 确认） | 推理日志、监控指标、网关 | Agent 原生 + 日志查询 + 模块诊断 | 一 |
...
| 自然语言查询（上月 GPU 消费等） | 资源使用统计 API | CRD 直操作（基座 LLM 问答） | 三 |
...
|---|---|---|
| **一 · 最小可用** | **开发环境**：自然语言创建环境、环境诊断；**推理**：智能部署、自动排障；**模型**：自动模型测试；**运维**：巡检自动化、智能日志分析 | 对话闭环 + Portal 对话页、Agent 实例与配置管理、平台资源操作 + 能力发现、定时 AI 任务（含巡检模板）、平台集成基础、可观测性接口 |
