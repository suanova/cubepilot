...
| FR-M3-006 | 平台能力目录：将平台能力（DevEnvironment / InferenceService / TrainingJob / Model / Dataset 等）形成 Agent 可发现、可调用的能力清单——每项含用途、关键参数、调用示例；作为实例级配置、启动时随 Agent 加载（能力说明注入 Agent 上下文，承接 FR-M2-005），Agent 据此正确选择并调用对应能力 | 能力目录随实例启动加载；Agent 依据目录正确选择并调用对应能力（如创建开发环境、部署推理服务）；新增能力仅需在目录补充条目、不改既有逻辑 | 一 |
...
|---|---|---|---|
| **一 · 首批必须** | 最小可用对话助手 | 对话闭环 + Portal 对话页（FR-M1-001、003~005、007）；Agent 实例与配置管理 + Runtime 适配层（FR-M2-002~005、009）；平台资源操作 + 能力目录（FR-M3-001、006）；**定时 AI 任务（含巡检模板）**（FR-M4-001~005、007）；平台集成基础（FR-M6-001~004）；可观测性接口（NFR-011~013）；安全/性能基础（NFR-002~004、006~007、010、016~017） | 自然语言创建开发环境/查询/操作推理服务（写操作直放、可借命令执行审批）；Agent 依据能力目录正确选择 CRD 并操作；实例常驻自愈；预置+AI 巡检每日出报告；/metrics+日志关联字段；首 Token<3s |
...
知识注入 → RAG（FR-M1-008）· Agent Runtime 适配层（FR-M2-009，一期落地 → 阶段三运行时替换）· 定时任务骨架（→ 推理服务验证/任务模板）· 平台操作底座 + 能力目录（→ 后续按需扩展非 K8s 工具）· **任务报告 → Agent 消费（阶段三 RCA/自动修复/预测运维，FR-M4-012~014）**。
...
| Q-010 | 能力目录（FR-M3-006）的维护流程：谁维护、模块如何接入（内容约定与审核）？ | §8.3、FR-M3-006 | 高 |
...
| 平台能力目录（各模块能力统一清单、实例级配置加载） | FR-M3-006 | 一 |
...
2. **能力目录**：模块能力纳入**能力目录**（FR-M3-006）——每项含用途、关键参数、调用示例；目录作为实例级配置启动时加载（承接 FR-M2-005）。
...
| **能力目录工具化** | 模块的**操作类**能力（创建环境、提交训练、部署推理等） | 模块提供 CRD/API，基座将能力纳入能力目录 |
| **模块自带** | 模块的**领域智能**（数据画像、模型推荐、评估解读、成本优化、告警关联等） | 由模块实现，基座提供对话/推理外壳 |

**§8.4 归属取值说明**（固定词汇，表内用「+」表示多方协作）：

- **能力目录工具化**：模块提供 CRD/API，能力纳入能力目录，Agent 经能力目录调用（纯问答类可标注「基座 LLM 问答」）；
...
> - **A. 能力 API 化**：模块封装领域逻辑为 API → 纳入能力目录 → Agent 工具化调用
