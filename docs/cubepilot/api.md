...
- 通常以 `message_done` 结束：回合到终态、客户端断开、或达到 1 小时上限。
  **例外**：门禁通过之后、订阅建立之前，那个人工决定已经被答掉了（回答端点是独立的，两者之间
  没有任何序列化）。续写的开头已经广播出去而无人接收，晚订的流补不回来——此时流会**不带终止事件**
  地结束：这是「本流什么都没观察到」的交代，客户端按常规自己合成 `message_done`，
  再从 history 把那一段补上。
- 不加热实例。
