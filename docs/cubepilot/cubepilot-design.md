...
spec:
  ...
  approvalPolicy: Allowlist                # 审批策略（None | Allowlist 默认 | AlwaysAsk）：命中安全 allowlist 自动放行，其余 interactive 操作需用户确认
...
generic 是默认能力：执行以实例 owner 的最小权限凭据进行（RBAC 是授权边界），schema 发现见 §5.3；平台不必为每个新 CRD 登记工具。写操作是否需确认由 AgentTemplate 的 `approvalPolicy`（§3.1）决定。
...
预置模板以 YAML 文件维护在 `internal/controller/presets/tasktemplates/`（一个文件一个模板，**文件名即 CR 名**），构建时嵌入、**严格解码**：字段名写错、键写重复都直接报错，而不是静默丢弃 —— 这是 Go 字面量原本由编译器给的保证，也是这次从代码改成文件的先决条件。`cubepilot/builtin` 等标签由加载器统一打，不写在每个文件里（「是否预置」是文件位置的性质，不是单文件的字段）。代价是文件形式多出一类 Go 代码不可能有的静默失败：**删掉一个文件照样构建成功，只是少一个预置**，所以有一份固定名单的测试盯着名单本身。
...
其中 `narration` 承载 agent 在工具之间说的话（"发现了什么、接下来做什么"）：网关的 `chat` 通道按设计**只发最终答复**，解说由网关自己改写为 `agent` 通道上的 `stream=item, kind=preamble` 事件（控制台 UI 读的就是它），投影器把它接成 `narration`。`text` 是整段快照、按 `blockId` 替换，且与答复字段严格分开——答复的三处既有语义（stopped 轮次判定、answer panel、"Final result" 标签）都只认 `text`。
...
- 读操作直放；写操作及未命中 allowlist 的命令在 interactive 会话命中确认即暂停（`Allowlist` 默认）。审批策略由 AgentTemplate 的 `approvalPolicy`（§3.1，`None | Allowlist` 默认 `| AlwaysAsk`）决定，实例可覆盖；`AlwaysAsk` 只把门槛抬到 interactive 会话内全部询问，`task-*`/`inspect-*` 等非 interactive 会话一律不 gate。
